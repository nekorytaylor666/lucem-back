...
const AppointmentTimetable: React.FC<AppointmentTimetableProps> = ({
  ...
  const workWeekScheduleDeclaration = () => {
    ...
    for (let weekIndex = 0; weekIndex < 3; weekIndex++) {
      const workWeekSchedule = convertWorkTimesToWorkWeekScheduleDeclaration(
        doctor?.workTimes,
        weekIndex
      );
          ...
          {workWeekSchedule[selectedDay]?.map((val, i) => {
            return (
              <TimeContainer
                ...
                isPicked={isSameHour(new Date(val.time.start), new Date(value))}
                setShow={() => console.log()}
              />
            );
          })}
  ...
  if (!doctor?.workTimes) {
    return (
      <div className="h-full">
        <p className="text-gray-400 mb-2">Нет доступного времени для записи</p>
        <div></div>
      </div>
    );
  }
