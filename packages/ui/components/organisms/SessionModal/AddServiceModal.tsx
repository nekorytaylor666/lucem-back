...
export const AddServiceModal: FC<AddServiceModalProps> = ({ onAddService }) => {
  ...
  const { data: doctorsData, loading: doctorLoading } = useGetDoctorSearchQuery(
    {
      onCompleted(data) {
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      {services.length > 0 && !servicesLoading ? (
        <div className="form-control">
          <label className="label">
            <span className="label-text  text-dark-grey">Название услуги</span>
          </label>
          <select
            onChange={handleChange}
      ...
      ></CategoryPick>
      {actionType.value === "booking" && doctors && doctors.length > 0 && (
        <div className="bg-white p-4">
          {doesDoctorHaveWorkTime ? (
            <AppointmentTimeTable
              value={values.bookingtime}
              onChange={(time) => setFieldValue("bookingtime", time)}
              doctor={doctor}
            ></AppointmentTimeTable>
          ) : (
      ...
      {actionType.value === "forward" && doctors && doctors.length > 0 && (
        <div className="bg-white p-4">
          <div className="form-control">
            <label className="label">
              <span className="label-text  text-dark-grey">Окончание</span>
            </label>
