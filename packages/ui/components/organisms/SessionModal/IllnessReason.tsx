...
export const IllnessReason: React.FC<SessionFormComponentProps> = () => {
  ...
  return (
    <FormAnimatedContainer>
      <h3 className="font-medium text-xl mb-4">Жалобы и анамнез заболевания</h3>
      <div className="w-full bg-base-200 rounded-lg p-4">
        <div>
          <span className=" text-xs text-base-300">
            Изменено: {parentValues?.complaint?.doctor?.fullName}
          </span>
        </div>
        <div className="form-control">
          ...
          <Field name={"complaint.complaint"} component={RichTextEditor} />
        </div>
        <div className="form-control">
          <label className="label">
