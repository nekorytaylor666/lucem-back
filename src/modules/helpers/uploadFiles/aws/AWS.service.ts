...
export class AWSservice {
  ...
  async saveImages(fileReadStream: ReadStream) {
    ...
    const resizedImages = await this.imageUploadService.imageResize(fileReadStream);
    const [thumbnailImage, mImage, xlImage] = await Promise.all([resizedImages[0], resizedImages[1], resizedImages[2]]) as Buffer[]
    await Promise.all([
        this.s3.putObject({
    ...
    const storageURL: string = this.configService.get('DIGITAL_OCEAN_SPACE_CDN') + keyStr;
